...
import type { CellKey, EditValidationResult, TimetableCell } from '@/entities/timetable'
...
export function validateCellEdit(
  ...
  if (targetCell && !isCellEditable(targetCell)) {
    violations.push({ type: 'NOT_EDITABLE', message: '고정 또는 잠긴 셀은 편집할 수 없습니다' })
  ...
  for (const cell of allCells) {
    const cellKey = makeCellKey(cell.grade, cell.classNumber, cell.day, cell.period)
  ...
  if (!isPlacementValid(grid, unit, day, period, constraintPolicy, blockedSlots, teacherPolicies)) {
    // 구체적 사유 분석
    if (grid.isTeacherBusy(newTeacherId, day, period)) {
      violations.push({ type: 'TEACHER_CONFLICT', message: `교사가 ${day} ${period}교시에 이미 배정되어 있습니다` })
    }
    const teacherBlockKey = `teacher-${newTeacherId}-${day}-${period}`
    if (blockedSlots.has(teacherBlockKey)) {
      violations.push({ type: 'BLOCKED_SLOT', message: `${day} ${period}교시는 차단된 슬롯입니다` })
    }
    ...
    if (grid.getTeacherDayHours(newTeacherId, day) >= maxDaily) {
      violations.push({ type: 'DAILY_LIMIT', message: `교사의 일일 최대 시수(${maxDaily})를 초과합니다` })
    }
    if (violations.length === 0) {
      violations.push({ type: 'CONSTRAINT_VIOLATION', message: '제약 조건 위반' })
    }
    return { valid: false, violations }
  }
...
export function validateCellMove(
  ...
  if (!sourceCell) {
    violations.push({ type: 'NOT_FOUND', message: '원본 셀을 찾을 수 없습니다' })
    return { valid: false, violations }
  }

  if (!isCellEditable(sourceCell)) {
    violations.push({ type: 'NOT_EDITABLE', message: '고정 또는 잠긴 셀은 이동할 수 없습니다' })
    return { valid: false, violations }
  }

  // 대상 위치에 이미 셀이 있는지 확인
  const targetKey = makeCellKey(source.grade, source.classNumber, targetDay, targetPeriod)
  const targetCell = allCells.find(
    (c) =>
      c.grade === source.grade &&
      c.classNumber === source.classNumber &&
  ...
  if (targetCell) {
    violations.push({ type: 'SLOT_OCCUPIED', message: `${targetDay} ${targetPeriod}교시에 이미 수업이 배정되어 있습니다` })
    return { valid: false, violations }
  }
