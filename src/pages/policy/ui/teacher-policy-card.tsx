...
export function TeacherPolicyCard({ teacher, policy, schoolConfig }: TeacherPolicyCardProps) {
  const {
    toggleAvoidanceSlot,
    setTimePreference,
  ...
  const isAvoided = (day: DayOfWeek, period: number): boolean => {
    return policy?.avoidanceSlots.some((s) => s.day === day && s.period === period) ?? false
  }
  ...
  const avoidanceCount = policy?.avoidanceSlots.filter(
    (s) => activeDays.includes(s.day) && s.period >= 1 && s.period <= periodsPerDay,
  ).length ?? 0
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-muted-foreground text-xs">
          과목: {teacher.subjectIds.length > 0 ? teacher.subjectIds.join(', ') : '미지정'} ·
          주당 {teacher.classAssignments.reduce((s, a) => s + a.hoursPerWeek, 0)}시수 ·
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-xs mb-2">클릭하여 회피 시간을 토글합니다</p>
          <div className="overflow-x-auto">
            <table className="text-xs">
              <thead>
                <tr>
                  <th className="pr-2 text-left text-muted-foreground font-normal">교시</th>
                  {activeDays.map((day) => (
                    <th key={day} className="px-1 text-center text-muted-foreground font-normal min-w-[2.5rem]">
                      {DAY_LABELS[day]}
                    </th>
                  ))}
                </tr>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="NONE">선호 없음</SelectItem>
